...
- `kb/data/toolchain.yaml` — language runtimes and ecosystem tooling, covering the base runtimes (`node`, `bun`, `python`, `uv`, `poetry`, `ruby`, `bundle`, `java`, `mvn`, `gradle`, `dotnet`, `php`, `composer`, `elixir`, `mix`) plus the Go developer toolchain (`gopls`, `dlv`, `goreleaser`, `air`, `mockgen`, `swag`, `wire`, `stringer`), the Node.js ecosystem (`corepack`, `deno`, `tsx`, `ts-node`, `nodemon`, bundlers `vite`/`webpack`/`rollup`/`esbuild`/`swc`/`parcel`, meta-frameworks `next`/`nuxt`/`react-scripts`, task runners `turbo`/`nx`/`lerna`/`gulp`/`grunt`, `pm2`, `node-gyp`), Python packaging and tooling (`uvx`, `pipenv`, `pdm`, `hatch`, `tox`, `nox`, `conda`, `mamba`, `virtualenv`, `pipx`, `ipython`, `jupyter`, `twine`, `pyinstaller`), the JVM build/JDK toolchain (`javac`, `javadoc`, `jar`, `jshell`, `keytool`, `mvnd`, `scala`/`scalac`/`sbt`/`scala-cli`/`mill`, `kotlin`/`kotlinc`/`kotlinc-jvm`/`kscript`, `ant`, `jbang`, `coursier`, `groovy`, `clojure`, `lein`) and Rust/Zig (`rustup`, `rustdoc`, `rust-analyzer`, `nextest`, the `cargo-*` family, `wasm-pack`, `trunk`, `cross`, `bacon`, `zig`, `zls`), dialect `pkgmgr`. The `pkgmgr` row of the command→dialect map below is the complete inventory.
- `kb/data/linting.yaml` — build/test/lint drivers across the JS/TS, Go, Python, Rust and shell toolchains (e.g. `tsc`, `eslint`, `prettier`, `vitest`, `jest`, `mocha`, `ava`, `biome`, `oxlint`, `gofmt`, `goimports`, `gofumpt`, `staticcheck`, `govulncheck`, `gosec`, `revive`, `ruff`, `black`, `mypy`, `pytest`, `flake8`, `pylint`, `pyright`, `coverage`, `sphinx`, `clippy-driver`, `cargo-clippy`, `rustfmt`, `shellcheck`), dialect `build`; the `build` row of the command→dialect map below is the complete inventory. (`golangci-lint` is deliberately left UNMODELLED — the verification marker tolerates it as an unresolved driver, and bounding it would silently clear the marker on the audited TRUE_DENY shape that rewrites the module graph; see `linting.yaml`.)
...
The expanded command sets are laid out one document per tradition, and every command in a document carries that tradition's dialect (a command belongs to exactly one dialect). The map below is the complete inventory of every command in the knowledge base (name → dialect); the data files remain the single source of truth, so the two must stay in lockstep. macOS variants of the process/system utilities stay under `bsd`, and `find`, `tar`, `git` and `rsync` keep their dedicated dialects.
...
- `pkgmgr` (`pkgmgr.yaml`, `toolchain.yaml`): air, ant, apk, apt, bacon, bun, bundle, cargo, cargo-audit, cargo-deny, cargo-expand, cargo-tarpaulin, cargo-watch, clojure, composer, conda, corepack, coursier, cross, deno, dlv, dnf, dotnet, dpkg, elixir, esbuild, flatpak, gem, go, gopls, goreleaser, gradle, groovy, grunt, gulp, hatch, ipython, jar, java, javac, javadoc, jbang, jshell, jupyter, keytool, kotlin, kotlinc, kotlinc-jvm, kscript, lein, lerna, mamba, mill, mix, mockgen, mvn, mvnd, next, nextest, node, node-gyp, nodemon, nox, npm, nuxt, nx, pacman, parcel, pdm, php, pip, pipenv, pipx, pm2, pnpm, poetry, pyinstaller, python, react-scripts, rollup, rpm, ruby, rust-analyzer, rustc, rustdoc, rustup, sbt, scala, scala-cli, scalac, snap, stringer, swag, swc, tox, trunk, ts-node, tsx, turbo, twine, uv, uvx, virtualenv, vite, wasm-pack, webpack, wire, yarn, yum, zig, zls, zypper.
- `build` (`build.yaml`, `plumbing.yaml`, `linting.yaml`): as, autopep8, ava, bandit, benchstat, biome, black, cargo-clippy, clippy-driver, cmake, cmp, concurrently, coverage, cross-env, cypress, diff, errcheck, eslint, file, flake8, gcc, gdb, ginkgo, go-junit-report, gofmt, gofumpt, goimports, gosec, gotestsum, govulncheck, ineffassign, isort, jest, ld, ltrace, make, mocha, mypy, ninja, nm, objdump, oxlint, patch, playwright, prettier, pydocstyle, pylint, pyright, pytest, readelf, revive, rimraf, ruff, rustfmt, serve, shellcheck, sphinx, staticcheck, strace, strings, strip, tsc, tsd, typedoc, uvu, valgrind, vitest.
