...
| Egress effects require a host-shaped target: a literal operand that names no network address creates no `NetEgress` effect | Medium | Deliberate precision/recall trade (2026-09, phantom-egress fix). Before the gate, the binder force-fit git subcommand names, commit SHAs and pathspecs onto network positionals (`git status` → `NetEgress{"status"}`), and consumers keying on the mere presence of a network effect fired on network-free commands; the secret-tainting rule then fabricated exfiltration pairs. The grammar (`engine.HostShaped`) accepts URLs, IPv4/IPv6 (incl. hex/decimal obfuscations), scp-style remotes and dotted DNS names; destination slots of declared network clients also accept single-label names (`engine.HostShapedLenient`). The safe side is preserved: an unresolved (⊤) target stays as *unresolved egress* and keeps participating in network controls, and `IsEgressSink` is kind-strict so a local file redirect can never be an exfiltration sink. Residual: a literal single-label remote alias in a strict dialect (`git fetch origin`) loses its egress effect — conservative in the deny direction only. |
